...
import { AppAjustes } from '../demo/service/appajustes.service'; // ← NUEVO
...
export class AppMenuLayoutComponent implements OnInit {
    ...
    constructor(
        ...
        private appAjustes: AppAjustes  // ← NUEVO
    ) { }

    ngOnInit() {
        const puedeVerAdmin = this.permissionService.canDoSync('soloAdminODueno');

        // Escuchar cambios en config y reconstruir el menú
        this.appAjustes.config$.subscribe(config => {
            const j1 = (config.espacioTrabajo.jerarquia1Nombre || 'Cliente').toUpperCase();
            const j2 = (config.espacioTrabajo.jerarquia2Nombre || 'Proyecto').toUpperCase();
            this.buildMenu(puedeVerAdmin, j1, j2);
        });
    }

    private buildMenu(puedeVerAdmin: boolean, j1Label: string, j2Label: string) {
        this.model = [
            {
            ...
            },
            {
                label: 'GESTIONAR',
                items: [
                    { label: j2Label, icon: 'pi pi-fw pi-file-pdf', routerLink: ['/menu-layout/proyectos'] }, // ← dinámico
                    { label: 'EQUIPO', icon: 'pi pi-fw pi-users', routerLink: ['/menu-layout/equipo'] },
                    { label: j1Label, icon: 'pi pi-fw pi-user', routerLink: ['/menu-layout/clientes'] },       // ← dinámico
